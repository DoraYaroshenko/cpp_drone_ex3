...
This plan details how to adapt your `cpp_drone_ex2` code to the `ex_3_skeleton`, adhering to all the assignment requirements (shared libraries, multithreading, specific folder structure). It is optimized for two people working independently on parallel branches.
...
- Move all files from Ex2 that are shared across Simulator, Algorithm, and MissionControl into `UserCommon`. This likely includes: `Beam`, `CollisionUtils`, `ScanResultToVoxels`, `ConfigParserUtils`, `Units`, `Types`, and `Logger`.
